...
router.get('/login/:aname/:apwd', (req, res)=>{
  ...
  //需要对用户输入的密码执行加密函数
  pool.query('SELECT aid FROM LittleVC_admin WHERE aname=? AND apwd=PASSWORD(?)', [aname, apwd], (err, result)=>{
    if(err)throw err;
...
router.patch('/', (req, res)=>{       
  ...
  pool.query('SELECT aid FROM LittleVC_admin WHERE aname=? AND apwd=PASSWORD(?)', [data.aname, data.oldPwd], (err, result)=>{
    if(err)throw err;
    if(result.length==0){
    ...
    //如果查询到了用户，再修改其密码
    pool.query('UPDATE LittleVC_admin SET apwd=PASSWORD(?) WHERE aname=?', [data.newPwd, data.aname], (err, result)=>{
        if(err)throw err;
