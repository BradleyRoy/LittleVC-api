...
*/
router.get('/', (req, res) => {
  pool.query('SELECT * FROM LittleVC_category ORDER BY cid', (err, result) => {
    if (err) throw err;
...
router.delete('/:cid', (req, res) => {
  ...
  pool.query('UPDATE LittleVC_dish SET categoryId=NULL WHERE categoryId=?', req.params.cid, (err, result) => {
    if (err) throw err;
    //至此指定类别的菜品已经修改完毕
    pool.query('DELETE FROM LittleVC_category WHERE cid=?', req.params.cid, (err, result) => {
      if (err) throw err;
...
router.post('/', (req, res)=>{
  ...
  pool.query('INSERT INTO LittleVC_category SET ?', data, (err, result)=>{  //注意此处SQL语句的简写
    if(err)throw err;
...
router.put('/', (req, res)=>{
  ...
  //TODO: 此处可以对数据数据进行验证
  pool.query('UPDATE LittleVC_category SET ? WHERE cid=?', [data, data.cid], (err, result)=>{
    if(err)throw err;
